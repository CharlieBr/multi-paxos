...
public class PathUtil {
    private static Pattern ROOT_PROJECT_PATTERN = Pattern.compile("^(.*?)\\/multi-paxos.*?$");
    ...
    static String getJarfile(String path) {
        ...
        File[] files = dir.listFiles(file -> file.getName().contains("node") && file.getName().endsWith("jar"));
        if (files.length != 1) {
